...
Crawler.prototype.getBemaniwikiMusicList = function() {
    var self = this;
    var wikiurl = 'http://bemaniwiki.com/index.php?jubeat%20saucer%2F%BF%B7%B6%CA%A5%EA%A5%B9%A5%C8';
    var flag = [false, false];

    this.download(wikiurl, {}, function(data) {
        var $ = cheerio.load(self.iconv('EUC-JP', 'UTF-8//TRANSLIT//IGNORE', data));
        $('tr').each(function(i,e) {
            if($(e).find('td').length==9) {
                var info = $(e).find('td').map(function(i,e){return $(e).text().trim()});
                ...
                self.emit('bemaniwiki.data',
                    info[0], info[1], info[2], difficulty, notecount);
            }
        });
        if( flag[1] ) 
            self.emit('bemaniwiki.end');
        flag[0] = true;
    });

    wikiurl = 'http://bemaniwiki.com/index.php?jubeat%20saucer%2F%B5%EC%B6%CA%A5%EA%A5%B9%A5%C8';
